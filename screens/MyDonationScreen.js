...
export default class MyDonationScreen extends Component {
   ...
   sendBook=(bookDetails)=>{
     if(bookDetails.request_status === "Exchange"){
       ...
       var requestStatus = "Exchange"
       db.collection("all_donations").doc(bookDetails.doc_id).update({
         "request_status" : "Exchange"
       })
       this.sendNotification(bookDetails,requestStatus)
     }
   }
       ...
       rightElement={
           <TouchableOpacity
            style={[
              styles.button,
              {
                backgroundColor : item.request_status === "Exchange" ? "green" : "#ff5722"
              }
            ]}
            onPress = {()=>{
              this.sendBook(item)
           ...
           >
             <Text style={{color:'#ffff'}}>{
               item.request_status === "Exchange" ? "Exchange" : "Exchange"
             }</Text>
           </TouchableOpacity>
         }
       bottomDivider
